...
byte AddressByteA = ADDRESS_BYTE_A;
byte AddressByteB = ADDRESS_BYTE_B;
        ...
        if (PacketLength >= 1) {
            ...
            if (recvPacket[start + 1] == 0x01 && recvPacket[start + 5] == 0xAA) { // If the bytes match an Ikea remote sequence
                ...
                if (USE_SERIAL) {
                    ...
                    if (AddressByteA < 0x10) {
                        Serial.print("0");
                    }
                    Serial.print(AddressByteA, HEX);
                    if (AddressByteB < 0x10) {
                        Serial.print("0");
                    }
